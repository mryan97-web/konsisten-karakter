...
export default function DashboardClient({
  ...
  // Fetch data from client side if server didn't provide initial data
  const loadData = async () => {
    ...
    try {
      const [userRes, charRes] = await Promise.all([
        ...
        fetch(`${base}/api/character`, { headers }),
      ]);
  ...
  const totalPrompts = characters.reduce((sum, c) => sum + c.prompt_count, 0);
  const remainingPrompts = user?.tier === 'free'
    ? Math.max(0, 30 - totalPrompts)
    : Infinity;
        ...
        <div className="mb-12 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-6">
            <div className="text-3xl font-bold">
              {user?.tier === 'free' ? remainingPrompts : '∞'}
            </div>
            ...
            {characters.map((char) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 text-xs text-[var(--muted)]">
                  ...
                  {char.type === 'custom' && (
                    <button
                      onClick={(e) => { e.stopPropagation(); setAnalyzeChar({ char_id: char.char_id, name: char.name, imageCount: 0 }); }}
