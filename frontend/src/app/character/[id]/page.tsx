    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="mb-8 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setShowEdit(true)} className="btn btn-secondary text-sm px-4 py-2">
                ✏️ Edit
              </button>
              <button
                onClick={() => setShowDeleteConfirm(true)}
                className="btn text-sm px-4 py-2 text-red-400 hover:bg-red-500/10 border border-red-500/30 rounded-xl transition-colors"
              >
                🗑️
              </button>
            </div>
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          <button
            ...
            className="flex flex-col items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 hover:border-[var(--primary)] transition-all hover:shadow-lg hover:shadow-[var(--primary)]/5 group"
          >
            <div className="mb-2 text-3xl group-hover:scale-110 transition-transform">
              {imageCount >= 5 ? '📸' : '📷'}
            </div>
            <div className="font-semibold text-sm">
              {imageCount > 0 ? `${imageCount} Foto` : 'Upload Foto'}
            </div>
            <div className="text-xs text-[var(--muted)]">{imageCount > 0 ? (imageCount >= 5 ? '✅ Minimal terpenuhi' : `${5 - imageCount} foto lagi`) : 'Min 5 foto'}</div>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 transition-all group
              ${imageCount >= 5 ? 'hover:border-[var(--primary)] hover:shadow-lg hover:shadow-[var(--primary)]/5 cursor-pointer' : 'opacity-50 cursor-not-allowed'}
            `}
          >
            <div className="mb-2 text-3xl group-hover:scale-110 transition-transform">🧬</div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {char.images.map((img) => (
                <div key={img.image_id} className="group relative aspect-square overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--muted-bg)]">
                  ...
                  <div className="absolute bottom-1 right-1 rounded bg-black/60 px-1.5 py-0.5 text-xs text-white/70">
                    #{img.sort_order}
                  </div>
                </div>
              ))}
          ...
          <div className="mb-8">
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-6">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {char.dna.base && (
                  <div>
                    <h4 className="text-xs font-medium text-[var(--primary)] uppercase tracking-wider mb-2">Base</h4>
                ...
                {char.dna.face && (
                  <div>
                    <h4 className="text-xs font-medium text-[var(--primary)] uppercase tracking-wider mb-2">Wajah</h4>
                    <div className="space-y-1 text-sm">
                      {Object.entries(char.dna.face).map(([k, v]) => (
                        <div key={k} className="flex gap-2">
                          <span className="text-[var(--muted)] min-w-[100px]">{k.replace(/_/g, ' ')}:</span>
                ...
                {char.dna.hair && (
                  <div>
                    <h4 className="text-xs font-medium text-[var(--primary)] uppercase tracking-wider mb-2">Rambut</h4>
                    <div className="space-y-1 text-sm">
                      {Object.entries(char.dna.hair).map(([k, v]) => (
                        <div key={k} className="flex gap-2">
                          <span className="text-[var(--muted)] min-w-[100px]">{k.replace(/_/g, ' ')}:</span>
                          <span>{String(v || '-')}</span>
                        </div>
        ...
        <div className="text-xs text-[var(--muted)] space-y-1">
          ...
          <div>ID: {char.char_id}</div>
          {char.share_code && <div>Share code: {char.share_code}</div>}
        </div>
      </main>
      ...
      {showEdit && (
        <EditCharacterModal
          ...
          isLocked={char.is_locked}
          onClose={() => setShowEdit(false)}
          onSaved={(data) => {
            setChar((prev) => prev ? { ...prev, ...data } : prev);
            setShowEdit(false);
          }}
        />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-xl border border-red-500/30 bg-[var(--card)] p-8 shadow-2xl">
            ...
            <p className="mb-6 text-center text-sm text-[var(--muted)]">
              Semua foto dan data DNA akan dihapus permanen. Tindakan ini tidak bisa dibatalkan.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className="btn btn-secondary flex-1 px-4 py-2.5">
                Batal
              </button>
              <button
